...
class Coroutine {
public:
	//Festlegen von Variablen die wichtig sind.

	struct setups{
		unsigned edi;	//register die flüchtig sind und gesichert werden müssen,
		unsigned esi; 	// same
		unsigned ebx;//same
		void *basepointer;	//Basepointer Adresse vom Start der Coroutine
		void (*start)(Coroutine *);//Startadresse
		void *jumpback;//Rücksprungadresse
		Coroutine *rut;	//die Coroutine um die es geht
	};
